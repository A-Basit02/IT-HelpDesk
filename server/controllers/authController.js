...
const register = async (req, res) => {
  const { name, email, password, employeeID, department, branch, role } = req.decryptedBody;

  if (!name || !email || !password || !employeeID || !department || !branch || !role) {
    return res.status(400).sendEncrypted({ message: 'All fields are required' });
  }
  ...
  try {
    ...
    res.status(500).sendEncrypted({ message: 'Server error', error: err.message });
  }
};
...
const login = async (req, res) => {
  try {
    ...
    if (!employeeID || !password) {
      return res.sendEncrypted({ message: 'employeeID and password are required' });
    }
...
const getLoggedInUser = async (req, res) => {
  try {
    ...
    const result = await request
      .input('employeeID', sql.VarChar, employeeID)
      .query('SELECT id, name, email, role, department, branch, employeeID FROM Users WHERE employeeID = @employeeID');
